...
public class PlayerData {
    ...
    public PlayerData(CaptchaPlugin plugin, Player player) {
        ...
        if (plugin.getStatusRecord().getUseSQL()) {
            plugin.getStatusRecord().setToSQL(player, 0, 0, false);
        } else {
            file = new File(
    ...
    public void handleSolveState(SolveState solveState) {
        if (solveState == SolveState.FAIL || solveState == SolveState.LEAVE) {
            ...
            if (plugin.getStatusRecord().getUseSQL()) {
                plugin.getStatusRecord().updateSolvedTime(player, lastPass);
            } else {
                fileConfiguration.set("last-pass", lastPass);
    ...
    public void fail() {
        if (plugin.getStatusRecord().getUseSQL()) {
            plugin.getStatusRecord().updateTotalFails(player);
        } else {
    ...
    public boolean hasPassed() {
        if (plugin.getStatusRecord().getUseSQL()) {
            return plugin.getStatusRecord().getPassed(player);
        } else {
