...
public class CaptchaHandler {
    ...
    public void assignCaptcha(Player player) throws IllegalStateException {
        ...
        try {
            ...
            player.kickPlayer(plugin.getMessageHandler().getMessage("no-maps", player.getLocale()));
            return;
        }
        new BukkitRunnable() {
            public void run() {
                notifyBungee(player, true);
                player.sendMessage(plugin.getMessageHandler().getMessage("join", player.getLocale()));
            }
    ...
    public void removeAssignedCaptcha(Player player, SolveState solveState) {
        ...
        if (solveState == SolveState.FAIL) {
            ...
            if (playerData.getFails() >= plugin.getConfig().getInt("captcha-settings.attempts")) {
                player.kickPlayer(plugin.getMessageHandler().getMessage("kick", player.getLocale()));
                return;
            }
    ...
    public void removeAllCaptchas() {
        for (Player onlinePlayer : Bukkit.getOnlinePlayers()) {
            ...
            if (playerData.hasAssignedCaptcha()) {
                ...
                onlinePlayer.kickPlayer(plugin.getMessageHandler().getMessage("reload-kick", onlinePlayer.getLocale()));
            }
        }
    }
