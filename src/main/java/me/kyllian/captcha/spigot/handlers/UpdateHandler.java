...
public class UpdateHandler {
    ...
    private final int RESOURCE_ID = 96373;
    ...
    public void handleUpdateMessage(Player player) {
        ...
        getLatestVersion(version -> {
            if (!version.equalsIgnoreCase(plugin.getDescription().getVersion())) player.sendMessage(plugin.getMessageHandler().getMessage("update", player.getLocale()));
        });
    }
}
